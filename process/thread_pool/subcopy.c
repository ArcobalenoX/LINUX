...
void *copydir(void* arg)
{
    ...
    src_dest path2 =(src_dest)arg;
    char* source_path=path2->src;
    char* destination_path=path2->dest;
    ...
    while(filename=readdir(sn))
    {
        ...
        src_dest path3=malloc(sizeof(struct src_dest));                                  
        strcpy(path3->src,file_source_path);
        strcpy(path3->dest,file_destination_path);
